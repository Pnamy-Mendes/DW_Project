...
const UserForm = ({ user, onSave, onCancel, userTypes }) => {
    const [localUser, setLocalUser] = useState({ ...user, typeUser: user.typeUser ? user.typeUser._id : null });

    useEffect(() => {
        setLocalUser({ ...user, typeUser: user.typeUser ? user.typeUser._id : null });
    }, [user]);
    ...
    return (
        <form className="p-fluid" onSubmit={handleSave}>
            ...
            <div className="field">
                ...
                <Dropdown
                    id="typeUser"
                    value={localUser.typeUser}
                    options={userTypes.map((type) => ({ label: type.name, value: type._id }))}
                    onChange={(e) => handleDropdownChange(e, 'typeUser')}
